...
export default function App() {
  ...
  // Client-side cache for fallback mode
  const clientCache = new Map<string, { data: any, expiresAt: number }>();
  const activeClientRequests = new Map<string, Promise<any>>();
  ...
  const proxyFetch = async (url: string, options: any = {}) => {
    try {
      ...
      const isHtmlProxyFallback = contentType.includes('text/html');

      // If the backend isn't there (e.g. static Cloudflare deployment), it'll return a 404, 405, or an HTML index page
      if (res.status === 404 || res.status === 405 || isHtmlProxyFallback) {
        console.warn(`[Proxy Missing] Static deployment detected. Falling back to direct client-side fetch for ${url}`);
        
        let finalUrl = url;
        // If nominatim, append email parameter to comply with openstreetmap terms since we can't set User-Agent in browsers
        if (finalUrl.includes('nominatim.openstreetmap.org')) {
          finalUrl += (finalUrl.includes('?') ? '&' : '?') + 'email=cerberus%40c0dejunky.com';
        }

        const cacheKey = `${options.method || 'GET'}:${finalUrl}:${parsedBody ? JSON.stringify(parsedBody) : ''}`;
        ...
        if (cached && cached.expiresAt > Date.now()) {
          return {
            ...
            text: async () => typeof cached.data === 'string' ? cached.data : JSON.stringify(cached.data),
            headers: new Headers(),
        ...
        if (activeClientRequests.has(cacheKey)) {
          return activeClientRequests.get(cacheKey);
        }

        const fetchPromise = (async () => {
          // Exclude User-Agent from direct fetch as browsers don't allow modifying it
          const directHeaders = { ...options.headers };
          if (directHeaders['User-Agent'] || directHeaders['user-agent']) {
            delete directHeaders['User-Agent'];
            delete directHeaders['user-agent'];
          }

          const directRes = await fetch(finalUrl, { ...options, headers: directHeaders });
          if (!directRes.ok) {
            return directRes;
          }
          
          const clonedRes = directRes.clone();
          const directContentType = directRes.headers.get('content-type');
          let data;
          if (directContentType && directContentType.includes('json')) {
             data = await clonedRes.json();
          } else {
             data = await clonedRes.text();
          }

          clientCache.set(cacheKey, { data, expiresAt: Date.now() + 60000 });
        ...
        activeClientRequests.set(cacheKey, fetchPromise);
        try {
          const result = await fetchPromise;
          activeClientRequests.delete(cacheKey);
          return result;
        } catch(e) {
          activeClientRequests.delete(cacheKey);
          throw e;
  ...
  const fetchTelemetryAnalysis = async (customTelemetry?: any, customCape?: number) => {
    ...
    try {
      ...
      logNetworkRequest({
        ...
        method: 'POST',
        status: isHtml ? 404 : res.status,
        statusText: (res.ok && !isHtml) ? 'OK' : 'Error',
      ...
      if (res.ok && !isHtml) {
        ...
        console.warn(`Server analyzer endpoint responded with error status: ${isHtml ? 'Static Fallback Page' : res.status}. Gemini features require backend execution.`);
      }
    } catch (e) {
